const DEFAULT_REMOTE_HOSTS: Record<string, string> = {
  ...
  full: 'https://wm.149.118.144.212.nip.io',
  world: 'https://wm.149.118.144.212.nip.io',
};
...
export function getRemoteApiBaseUrl(): string {
  ...
  return DEFAULT_REMOTE_HOSTS[variant] ?? DEFAULT_REMOTE_HOSTS.full ?? 'https://wm.149.118.144.212.nip.io';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.worldmonitor.app') || host === 'wm.149.118.144.212.nip.io';
  } catch {
    return false;
  }
}
